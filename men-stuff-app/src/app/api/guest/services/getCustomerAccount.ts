...
export async function getCustomerAccount() {
  ...
  const { data, error } = await getSupabase().from('customer').select('*').eq('account_id', accountId).single()
...
export async function getCurrentCustomerId() {
  ...
  const { data, error } = await getSupabase().from('customer').select('*').eq('account_id', accountId)
